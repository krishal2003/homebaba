...
export const _games = ['Valorant', 'PUB-G Mobile', 'Clash Of Clans'].map((game, index) => ({
  id: _mock.id(index),
  name: game,
  avatar: _mock.image.avatar(index),
  favourite: randomNumberRange(1, 19),
}));
...
export const _tournamentFeatured = [...Array(4)].map((_, index) => ({
  id: _mock.id(index),
  title: [
    'Valorant at Locus 2023',
    'Fifa 22 - Free Entry',
    'PubG Mobile - Exclusive meet with DRS Gaming',
    'PubG PC - Shroud Guest Entry',
  ][index],
  description: _mock.text.title(index),
  image: `/assets/images/home/featured_tournaments_${index + 1}.jpg`,
}));
